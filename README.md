...
"property not found". Yaml-Config-compare is utilized to prevent this situation.
...
yaml-config-compare
# yaml-config-compare --file1 "config.yaml" --file2 "config-sample.yaml"

# run where the config files are in a directory (ex: deployment)
# yaml-config-compare --dir "deployment" --file1 "config.yaml" --file2 "config-sample.yaml"
```
